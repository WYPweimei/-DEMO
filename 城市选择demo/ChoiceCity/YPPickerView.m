...
- (void)setupDefault
{
    ...
    _heightPicker      = 240;
    _contentMode       = YPPickerContentModeBottom;
...
- (void)layoutSubviews
{
    ...
    if (self.contentMode == YPPickerContentModeBottom) {
    }else {
        self.buttonLeft.y = self.lineViewDown.bottom + YPMarginSmall;
        self.buttonRight.y = self.lineViewDown.bottom + YPMarginSmall;
    }
}
...
- (void)setContentMode:(YPPickerContentMode)contentMode
{
    _contentMode = contentMode;
    if (contentMode == YPPickerContentModeCenter) {
        self.contentView.height += YPControlSystemHeight;
    }
}
...
- (UIView *)lineView
{
    if (!_lineView) {
        CGFloat lineX = 0;
        CGFloat lineY = YPControlSystemHeight;
...
{
    if (!_buttonLeft) {
        CGFloat leftW = YPControlSystemHeight;
        CGFloat leftH = self.lineView.top - YPMargin;
        CGFloat leftX = YPMarginBig;
...
{
    if (!_labelTitle) {
        CGFloat titleX = self.buttonLeft.right + YPMarginSmall;
        CGFloat titleY = 0;
